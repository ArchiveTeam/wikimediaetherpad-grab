...
LUA_SHA1 = get_hash(os.path.join(CWD, 'wikimediaetherpad.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--content-on-error',
            '--lua-script', 'wikimediaetherpad.lua',
            ...
            '--connect-timeout', '1',
            '--tries', 'inf',
            '--domains', 'etherpad.wikimedia.org,etherpad-backup.toolforge.org',
            '--span-hosts',
            '--waitretry', '30',
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'pad':
                wget_args.extend(['--warc-header', 'wikimedia-etherpad-pad: '+item_value])
                wget_args.append('https://etherpad.wikimedia.org/p/{}/export/etherpad'.format(item_value))
            else:
